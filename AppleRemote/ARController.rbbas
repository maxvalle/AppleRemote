		...
		Private Function initializeCookies() As boolean
		    ...
		    if success = kIOReturnSuccess then
		      ...
		      for i as integer = 0 to n
		        ...
		        // Get cookie
		        obj = CFDictionaryGetValue(element, _CFSTR(kIOHIDElementCookieKey))
		        if obj <> 0 and CFGetTypeID(obj) = CFNumberGetTypeID() then
		          if CFNumberGetValue(obj, kCFNumberLongType, number) then
		            cookie = number
		            
		            //Get usage
		            obj = CFDictionaryGetValue(element, _CFSTR(kIOHIDElementUsageKey))
		            if obj <> 0 and CFGetTypeID(obj) = CFNumberGetTypeID() then
		              if CFNumberGetValue(obj, kCFNumberLongType, number) then
		                usage = number
		                
		                //Get usage page
		                obj = CFDictionaryGetValue(element, _CFSTR(kIOHIDElementUsagePageKey))
		                if obj <> 0 and CFGetTypeID(obj) = CFNumberGetTypeID() then
		                  if CFNumberGetValue(obj, kCFNumberLongType, number) then
		                    usagePage = number
		    ...
		    if ioReturnValue = KERN_SUCCESS then
		      ...
		      if queue <> nil then
		        ...
		        if ioReturnValue = KERN_SUCCESS then
		          ...
		          if ioReturnValue = KERN_SUCCESS then
		            CFRunLoopAddSource(CFRunLoopGetCurrent(), eventSource, _CFSTR(kCFRunLoopDefaultMode))
	...
	#tag Method, Flags = &h0
		Sub stopListening()
		    ...
		    if eventSource <> 0 then
		      CFRunLoopRemoveSource CFRunLoopGetCurrent(), eventSource, _CFSTR(kCFRunLoopDefaultMode)
